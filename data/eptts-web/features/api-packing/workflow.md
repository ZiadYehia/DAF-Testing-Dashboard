# Packing (Aggregation) — API Workflow

## Feature Details

| Field | Value |
|-------|-------|
| **Feature Name** | Packing (Aggregation) |
| **Slug** | `api-packing` |
| **Feature ID** | `EPTTS_API_03` |
| **Module** | EPTTS APIs |
...
**Distinctive fields** — `parentID`: the SSCC · `childEPCs`: the SGTINs being added

**EPC carrier** — parentID + childEPCs (not epcList)
...
Final state: the SSCC contains the children; each child becomes **Packed**.
