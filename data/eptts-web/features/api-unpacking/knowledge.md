# api-unpacking — Feature Knowledge

## Why this feature matters

The mirror of packing, and distinguished from it only by `action: DELETE` and `bizStep: unpacking`. That similarity is exactly why it needs its own coverage: an implementation that confuses the two directions would still "succeed".

## Contract specifics

- `AggregationEvent`, `action: DELETE`, `bizStep: unpacking`, `parentID` + `childEPCs`.
- A removed child reverts to standalone; assert `pack.parentSscc` becomes null rather than trusting
  the message status.

## What will bite you

- Assert the **direction** of the change, not just SUCCESS. A test that only checks the terminal
...
- **TS_UNPK_011** — https://dafholding.atlassian.net/browse/DW-858 _(DW-858)_

### Verification status

**Not yet executed against production.** Every case is `Under Testing` / `new_added`.

The source spreadsheet recorded these statuses against **staging**, by a different tester: Fail 1, Pass 10. Those are retained here as history only — they are not evidence of coverage in this environment, so they are deliberately not carried into the Status column.
