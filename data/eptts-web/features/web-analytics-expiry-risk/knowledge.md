# web-analytics-expiry-risk — Feature Knowledge
...
- This is a **tab**, not a route: it lives at `/analytics` and is only reachable by clicking **Expiry risk**. There is no deep link, so a test must navigate then click.
