# web-reporting — Feature Knowledge

## Why this feature matters

Partners reconcile their own records against this page's CSV export, so column fidelity and filter correctness matter as much as the on-screen totals. An export that silently drops a filter is worse than one that fails.
...
- Always verify the export **against the filtered on-screen rows**, not just that a file downloads.
