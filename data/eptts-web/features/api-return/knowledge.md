# api-return — Feature Knowledge

## Why this feature matters

Returns send stock back upstream. Structurally this is shipping with `disposition: returned`, but the business rule differs in a way that matters: a return must trace back to the partner that originally supplied the pack, not to an arbitrary GLN.

## Contract specifics

- `ObjectEvent`, `action: OBSERVE`, `bizStep: shipping`, `disposition: returned`, with a return
...
- Returning to a GLN that never shipped the pack is the key negative case.
...
- **TS_RTN_005** — https://dafholding.atlassian.net/browse/DW-881 _(DW-881)_

### Verification status

**Not yet executed against production.** Every case is `Under Testing` / `new_added`.

The source spreadsheet recorded these statuses against **staging**, by a different tester: Fail 1, Pass 41. Those are retained here as history only — they are not evidence of coverage in this environment, so they are deliberately not carried into the Status column.
