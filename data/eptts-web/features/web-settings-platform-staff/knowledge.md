# web-settings-platform-staff — Feature Knowledge
...
- This is a **tab**, not a route: it lives at `/admin` and is only reachable by clicking **Platform Staff**. There is no deep link, so a test must navigate then click.
