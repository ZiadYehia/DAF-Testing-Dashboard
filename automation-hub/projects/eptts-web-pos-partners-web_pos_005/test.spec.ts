/**
 * WEB_POS_005 — Validate that a non-privileged role cannot reach this page
 *
 * Feature: web-pos-partners   Route: /pos-partners
 *
...
test('WEB_POS_005 — Validate that a non-privileged role cannot reach this page', async ({ browser }) => {
  test.slow()
  ...
  try {
    await page.goto(baseUrl('eptts-web') + '/pos-partners', { waitUntil: 'domcontentloaded', timeout: 45_000 })
    ...
    const refused = landed !== '/pos-partners' ||
      /unauthori[sz]ed|forbidden|access denied|not permitted|no permission/i.test(text)

    expect(
      refused,
      `a manufacturer must not reach /pos-partners — landed on ${landed}`,
    ).toBe(true)
  } finally {
    await ctx.close()
  }
