/**
 * WEB_AGD_005 — Validate that a non-privileged role cannot reach this page
 *
...
test('WEB_AGD_005 — Validate that a non-privileged role cannot reach this page', async ({ browser }) => {
  test.slow()
  ...
  try {
    await page.goto(baseUrl('eptts-web') + '/agent-devices', { waitUntil: 'domcontentloaded', timeout: 45_000 })
    ...
    const refused = landed !== '/agent-devices' ||
      /unauthori[sz]ed|forbidden|access denied|not permitted|no permission/i.test(text)

    expect(
      refused,
      `a manufacturer must not reach /agent-devices — landed on ${landed}`,
    ).toBe(true)
  } finally {
    await ctx.close()
  }
