/**
 * WEB_SMF_007 — Validate that Manufacturer rejects a GLN with an invalid check digit
 *
...
test('WEB_SMF_007 — Validate that Manufacturer rejects a GLN with an invalid check digit', async ({ page }) => {
  test.slow()
  await DashboardPage.openTab(page, '/admin', 'Manufacturer')
    .expectEnglish()
    .expectCreateFormRejectsBadGln('Add Manufacturer')
})
