/**
 * WEB_AMD_005 — Validate that Master-data changes rejects a GLN with an invalid check digit
 *
...
test('WEB_AMD_005 — Validate that Master-data changes rejects a GLN with an invalid check digit', async ({ page }) => {
  test.slow()
  await DashboardPage.openTab(page, '/audit', 'Master-data changes')
    .expectEnglish()
    .expectGlnFilterRejectsBadCheckDigit('GLN or')
})
