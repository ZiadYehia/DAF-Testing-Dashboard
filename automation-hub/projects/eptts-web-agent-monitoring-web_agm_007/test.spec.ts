/**
 * WEB_AGM_007 — Validate that a non-privileged role cannot reach this page
 *
...
test('WEB_AGM_007 — Validate that a non-privileged role cannot reach this page', async ({ browser }) => {
  test.slow()
  ...
  try {
    await page.goto(baseUrl('eptts-web') + '/agent-monitoring', { waitUntil: 'domcontentloaded', timeout: 45_000 })
    ...
    const refused = landed !== '/agent-monitoring' ||
      /unauthori[sz]ed|forbidden|access denied|not permitted|no permission/i.test(text)

    expect(
      refused,
      `a manufacturer must not reach /agent-monitoring — landed on ${landed}`,
    ).toBe(true)
  } finally {
    await ctx.close()
  }
