/**
 * WEB_RPK_005 — Validate that a non-privileged role cannot reach this page
 *
 * Feature: web-repack   Route: /repack-ingest
 *
...
test('WEB_RPK_005 — Validate that a non-privileged role cannot reach this page', async ({ browser }) => {
  test.slow()
  ...
  try {
    await page.goto(baseUrl('eptts-web') + '/repack-ingest', { waitUntil: 'domcontentloaded', timeout: 45_000 })
    ...
    const refused = landed !== '/repack-ingest' ||
      /unauthori[sz]ed|forbidden|access denied|not permitted|no permission/i.test(text)

    expect(
      refused,
      `a manufacturer must not reach /repack-ingest — landed on ${landed}`,
    ).toBe(true)
  } finally {
    await ctx.close()
  }
