/**
 * WEB_PHS_006 — Validate that a non-privileged role cannot reach this page
 *
...
test('WEB_PHS_006 — Validate that a non-privileged role cannot reach this page', async ({ browser }) => {
  test.slow()
  ...
  try {
    await page.goto(baseUrl('eptts-web') + '/pharmacy-stock', { waitUntil: 'domcontentloaded', timeout: 45_000 })
    ...
    const refused = landed !== '/pharmacy-stock' ||
      /unauthori[sz]ed|forbidden|access denied|not permitted|no permission/i.test(text)

    expect(
      refused,
      `a manufacturer must not reach /pharmacy-stock — landed on ${landed}`,
    ).toBe(true)
  } finally {
    await ctx.close()
  }
