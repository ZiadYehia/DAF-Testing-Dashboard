/**
 * WEB_MDM_006 — Validate that a non-privileged role cannot reach this page
 *
...
test('WEB_MDM_006 — Validate that a non-privileged role cannot reach this page', async ({ browser }) => {
  test.slow()
  ...
  try {
    await page.goto(baseUrl('eptts-web') + '/mdm-registry', { waitUntil: 'domcontentloaded', timeout: 45_000 })
    ...
    const refused = landed !== '/mdm-registry' ||
      /unauthori[sz]ed|forbidden|access denied|not permitted|no permission/i.test(text)

    expect(
      refused,
      `a manufacturer must not reach /mdm-registry — landed on ${landed}`,
    ).toBe(true)
  } finally {
    await ctx.close()
  }
