/**
 * WEB_DSP_005 — Validate that a non-privileged role cannot reach this page
 *
...
test('WEB_DSP_005 — Validate that a non-privileged role cannot reach this page', async ({ browser }) => {
  test.slow()
  ...
  try {
    await page.goto(baseUrl('eptts-web') + '/dispensing', { waitUntil: 'domcontentloaded', timeout: 45_000 })
    ...
    const refused = landed !== '/dispensing' ||
      /unauthori[sz]ed|forbidden|access denied|not permitted|no permission/i.test(text)

    expect(
      refused,
      `a manufacturer must not reach /dispensing — landed on ${landed}`,
    ).toBe(true)
  } finally {
    await ctx.close()
  }
