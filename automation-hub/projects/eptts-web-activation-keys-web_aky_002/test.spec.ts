/**
 * WEB_AKY_002 — Validate that /activation-keys is reachable by direct URL and survives a refresh
 *
...
test('WEB_AKY_002 — Validate that /activation-keys is reachable by direct URL and survives a refresh', async ({ page }) => {
  test.slow()
  await DashboardPage.open(page, '/activation-keys')
    .expectEnglish()
    .expectSurvivesReload('/activation-keys')
    .expectNoErrorBanner()
})
