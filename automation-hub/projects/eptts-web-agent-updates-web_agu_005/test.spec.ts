/**
 * WEB_AGU_005 — Validate that a non-privileged role cannot reach this page
 *
 * Feature: web-agent-updates   Route: /agent-updates
 *
...
test('WEB_AGU_005 — Validate that a non-privileged role cannot reach this page', async ({ browser }) => {
  test.slow()
  ...
  try {
    await page.goto(baseUrl('eptts-web') + '/agent-updates', { waitUntil: 'domcontentloaded', timeout: 45_000 })
    ...
    const refused = landed !== '/agent-updates' ||
      /unauthori[sz]ed|forbidden|access denied|not permitted|no permission/i.test(text)

    expect(
      refused,
      `a manufacturer must not reach /agent-updates — landed on ${landed}`,
    ).toBe(true)
  } finally {
    await ctx.close()
  }
