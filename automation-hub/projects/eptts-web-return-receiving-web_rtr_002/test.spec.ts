/**
 * WEB_RTR_002 — Validate that /pending-returns is reachable by direct URL and survives a refresh
 *
...
test('WEB_RTR_002 — Validate that /pending-returns is reachable by direct URL and survives a refresh', async ({ page }) => {
  test.slow()
  await DashboardPage.open(page, '/pending-returns')
    .expectEnglish()
    .expectSurvivesReload('/pending-returns')
    .expectNoErrorBanner()
})
