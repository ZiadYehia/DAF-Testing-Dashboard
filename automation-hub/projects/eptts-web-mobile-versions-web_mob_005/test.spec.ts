/**
 * WEB_MOB_005 — Validate that a non-privileged role cannot reach this page
 *
...
test('WEB_MOB_005 — Validate that a non-privileged role cannot reach this page', async ({ browser }) => {
  test.slow()
  ...
  try {
    await page.goto(baseUrl('eptts-web') + '/mobile-versions', { waitUntil: 'domcontentloaded', timeout: 45_000 })
    ...
    const refused = landed !== '/mobile-versions' ||
      /unauthori[sz]ed|forbidden|access denied|not permitted|no permission/i.test(text)

    expect(
      refused,
      `a manufacturer must not reach /mobile-versions — landed on ${landed}`,
    ).toBe(true)
  } finally {
    await ctx.close()
  }
