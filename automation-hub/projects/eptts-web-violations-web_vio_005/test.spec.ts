/**
 * WEB_VIO_005 — Validate that a non-privileged role cannot reach this page
 *
...
test('WEB_VIO_005 — Validate that a non-privileged role cannot reach this page', async ({ browser }) => {
  test.slow()
  ...
  try {
    await page.goto(baseUrl('eptts-web') + '/violations', { waitUntil: 'domcontentloaded', timeout: 45_000 })
    ...
    const refused = landed !== '/violations' ||
      /unauthori[sz]ed|forbidden|access denied|not permitted|no permission/i.test(text)

    expect(
      refused,
      `a manufacturer must not reach /violations — landed on ${landed}`,
    ).toBe(true)
  } finally {
    await ctx.close()
  }
