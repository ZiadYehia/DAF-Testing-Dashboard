/**
 * WEB_MDT_005 — Validate that a non-privileged role cannot reach this page
 *
...
test('WEB_MDT_005 — Validate that a non-privileged role cannot reach this page', async ({ browser }) => {
  test.slow()
  ...
  try {
    await page.goto(baseUrl('eptts-web') + '/master-data', { waitUntil: 'domcontentloaded', timeout: 45_000 })
    ...
    const refused = landed !== '/master-data' ||
      /unauthori[sz]ed|forbidden|access denied|not permitted|no permission/i.test(text)

    expect(
      refused,
      `a manufacturer must not reach /master-data — landed on ${landed}`,
    ).toBe(true)
  } finally {
    await ctx.close()
  }
