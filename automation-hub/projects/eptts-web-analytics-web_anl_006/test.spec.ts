/**
 * WEB_ANL_006 — Validate that a non-privileged role cannot reach this page
 *
 * Feature: web-analytics   Route: /analytics
 *
...
test('WEB_ANL_006 — Validate that a non-privileged role cannot reach this page', async ({ browser }) => {
  test.slow()
  ...
  try {
    await page.goto(baseUrl('eptts-web') + '/analytics', { waitUntil: 'domcontentloaded', timeout: 45_000 })
    ...
    const refused = landed !== '/analytics' ||
      /unauthori[sz]ed|forbidden|access denied|not permitted|no permission/i.test(text)

    expect(
      refused,
      `a manufacturer must not reach /analytics — landed on ${landed}`,
    ).toBe(true)
  } finally {
    await ctx.close()
  }
