/**
 * WEB_SPA_007 — Validate that Pharmacy Admins rejects a GLN with an invalid check digit
 *
...
test('WEB_SPA_007 — Validate that Pharmacy Admins rejects a GLN with an invalid check digit', async ({ page }) => {
  test.slow()
  await DashboardPage.openTab(page, '/admin', 'Pharmacy Admins')
    .expectEnglish()
    .expectCreateFormRejectsBadGln('Add Pharmacy Admin')
})
