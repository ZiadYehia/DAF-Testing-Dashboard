/**
 * WEB_RPT_008 — Validate that a non-privileged role cannot reach this page
 *
...
test('WEB_RPT_008 — Validate that a non-privileged role cannot reach this page', async ({ browser }) => {
  test.slow()
  ...
  try {
    await page.goto(baseUrl('eptts-web') + '/reporting', { waitUntil: 'domcontentloaded', timeout: 45_000 })
    ...
    const refused = landed !== '/reporting' ||
      /unauthori[sz]ed|forbidden|access denied|not permitted|no permission/i.test(text)

    expect(
      refused,
      `a manufacturer must not reach /reporting — landed on ${landed}`,
    ).toBe(true)
  } finally {
    await ctx.close()
  }
