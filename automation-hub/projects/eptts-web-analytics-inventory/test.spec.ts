/**
 * WEB_NIV_001 — Inventory renders with its content and primary controls.
 *
 * Feature: web-analytics-inventory   Route: /analytics
 *
...
test('WEB_NIV_001 — Inventory renders', async ({ page }) => {
  test.slow()
  await DashboardPage.openTab(page, '/analytics', 'Inventory')
    .expectEnglish()
    .expectControls(['Refresh', 'Activity', 'Inventory'])
    .expectTableColumns(['LOCATION', 'PRODUCT', 'GTIN', 'BATCH', 'EXPIRY'])
