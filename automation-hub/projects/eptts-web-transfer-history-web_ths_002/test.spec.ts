/**
 * WEB_THS_002 — Validate that /shipments/history is reachable by direct URL and survives a refresh
 *
...
test('WEB_THS_002 — Validate that /shipments/history is reachable by direct URL and survives a refresh', async ({ page }) => {
  test.slow()
  await DashboardPage.open(page, '/shipments/history')
    .expectEnglish()
    .expectSurvivesReload('/shipments/history')
    .expectNoErrorBanner()
})
