/**
 * WEB_AGM_002 — Validate that /agent-monitoring is reachable by direct URL and survives a refresh
 *
...
test('WEB_AGM_002 — Validate that /agent-monitoring is reachable by direct URL and survives a refresh', async ({ page }) => {
  test.slow()
  await DashboardPage.open(page, '/agent-monitoring')
    .expectEnglish()
    .expectSurvivesReload('/agent-monitoring')
    .expectNoErrorBanner()
})
