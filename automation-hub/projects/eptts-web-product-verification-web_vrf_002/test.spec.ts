/**
 * WEB_VRF_002 — Validate that /verify is reachable by direct URL and survives a refresh
 *
...
test('WEB_VRF_002 — Validate that /verify is reachable by direct URL and survives a refresh', async ({ page }) => {
  test.slow()
  await DashboardPage.open(page, '/verify')
    .expectEnglish()
    .expectSurvivesReload('/verify')
    .expectNoErrorBanner()
})
