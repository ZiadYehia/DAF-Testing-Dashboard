/**
 * WEB_SPL_001 — Platform renders with its content and primary controls.
 *
 * Feature: web-settings-platform   Route: /admin
 *
...
test('WEB_SPL_001 — Platform renders', async ({ page }) => {
  test.slow()
  await DashboardPage.openTab(page, '/admin', 'Platform')
    .expectEnglish()
    .expectControls(['New platform user'])
    .expectTableColumns(['EMAIL', 'NAME', 'ROLE', 'STATUS', 'LAST LOGIN'])
