/**
 * WEB_RTS_003 — Validate that a non-privileged role cannot reach this page
 *
 * Feature: web-return-shipping   Route: /return-to-branch
 *
...
test('WEB_RTS_003 — Validate that a non-privileged role cannot reach this page', async ({ browser }) => {
  test.slow()
  ...
  try {
    await page.goto(baseUrl('eptts-web') + '/return-to-branch', { waitUntil: 'domcontentloaded', timeout: 45_000 })
    ...
    const refused = landed !== '/return-to-branch' ||
      /unauthori[sz]ed|forbidden|access denied|not permitted|no permission/i.test(text)

    expect(
      refused,
      `a manufacturer must not reach /return-to-branch — landed on ${landed}`,
    ).toBe(true)
  } finally {
    await ctx.close()
  }
